...
class App:
    ...
    def interface_principal(self):
        ...
        ttk.Button(self.root, text="Cadastro", command=self.janela_cadastro).pack(pady=10)

    def interface_emprestimo(self):
        frame = self.aba_emprestimo

        if self.usuario_logado[2] not in ('adm', 'professor'):
    ...
    def realizar_emprestimo(self):
        ...
        if not patrimonio or not prazo or not aluno_matricula:
        ...
        emprestar_notebook(patrimonio, aluno_matricula, self.usuario_logado[0], prazo)
    ...
    def janela_cadastro(self):
        if self.usuario_logado[2] != 'adm':
            messagebox.showerror("Acesso negado", "Somente administradores podem acessar o cadastro.")
            return
